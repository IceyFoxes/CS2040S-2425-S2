...
public class GameTree {
    ...
    int findValue() {
        // TODO: Implement this
        // returns max child for Player One and min child for Player Two
        if (root == null) {
            return Integer.MIN_VALUE;
        }
        return findValue(root, true);
    }

    int findValue(TreeNode node, boolean isPlayerOne) {
        if (node.leaf) {
            return node.value;
        }
        ...
        if (isPlayerOne) {
            ...
            for (int i = 0; i < node.numChildren; i++) {
                if (node.children[i] != null) {
                    int childValue = findValue(node.children[i], false);
            ...
            for (int i = 0; i < node.numChildren; i++) {
                if (node.children[i] != null) {
                    int childValue = findValue(node.children[i], true);
